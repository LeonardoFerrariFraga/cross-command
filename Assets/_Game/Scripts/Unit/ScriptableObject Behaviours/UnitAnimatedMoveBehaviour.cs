...
public class UnitAnimatedMoveBehaviour : ScriptableObject
{
    ...
    public async Task AnimatedMove(Transform transformMove, Transform transformAnimate, Vector3 direction, CancellationToken cancellationToken) {
        Task move = _data.moveBehaviour.Move(transformMove, direction, _data.moveData, cancellationToken);
        Task jump = _data.jumpBehaviour.Jump(transformAnimate, _data.jumpData, cancellationToken);
